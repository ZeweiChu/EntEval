...
class PreCoEval(object):
    def __init__(self, taskpath, seed=1111):
        logging.debug('***** Transfer task : PreCo Binary Classification*****\n\n')
        self.seed = seed
        train = self.loadFile(os.path.join(taskpath, 'train.txt'))
        valid = self.loadFile(os.path.join(taskpath, 'valid.txt'))
    ...
    def loadFile(self, fpath):
        ...
        item = []
        i = 1
        with codecs.open(fpath, 'rb', 'latin-1') as fin:
            for line in fin:
                if line.strip() == "":
                    data.append(item)
                    item = []
